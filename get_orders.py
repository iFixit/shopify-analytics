...
start_date = datetime.datetime.now(tz=tzoffset) - datetime.timedelta(
    days=days_ago, minutes=minutes_ago
)
print("Fetching orders since", start_date.isoformat())

get_orders_from_start_date = partial(
    shopify.Order.find, status="any", updated_at_min=start_date.isoformat()
)
orders_res = get_all_orders(get_orders_from_start_date)

process_order = lambda order: convert_types(set_order_id(order))
all_orders = map(process_order, map(lambda order: order.to_dict(), orders_res))
